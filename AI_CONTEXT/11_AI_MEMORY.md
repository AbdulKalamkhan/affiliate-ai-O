...
CONTEXT: WEB TEST COVERAGE + HTTP SECURITY HEADERS pass
EXPECTED: Money-display logic regression-tested; web hardened at the edge without breaking SSR or inline styles
ACTUAL: apps/web had a no-op 'test' script (echo) - zero real assertions on the one place where fabricated money could appear. Extracted the duplicated inr()/rate math into app/_ui/format.ts (output byte-identical) and covered it with 7 jest unit tests asserting the honest-fallback contracts (null rate without clicks>0 + conversions; 'Awaiting data'; '—' for unknown money; en-IN grouping). next.config.ts now sets nosniff / Referrer-Policy / Permissions-Policy / X-Frame-Options DENY / CSP (connect-src pinned to the API origin; script+style keep unsafe-inline because these are server components with inline styles - a nonce-based strict CSP was deliberately NOT attempted) / HSTS, and poweredByHeader:false. All 7 verified live. Added a glass not-found page. Commits cbdf87e, 6cacfb7, 4b7b960.
