...
- PHASE-02 core (Owner MAXIMUM-AUTONOMY directive 2026-09-25 — "continue through every phase"): Phase-02 = "Build evidence-backed opportunity intelligence" — research sources/evidence, trend signals, opportunity normalization, scoring. Gate: repeatable evidence-backed ranked opportunities. CORE IMPLEMENTED + TESTED 2026-09-25: opportunity_evidence table (additive migration `20260925130000_add_opportunity_evidence`), OpportunityIntelligenceModule (POST/GET /opportunity-intelligence/opportunities/:id/evidence, GET /opportunity-intelligence/opportunities/:id/score, GET /opportunity-intelligence/ranked), deterministic versioned weight set (SCORE_WEIGHTS_VERSION=1), per-claim quality classes FACT/ESTIMATE/INFERENCE/PREDICTION/UNKNOWN with QUALITY_WEIGHT (UNKNOWN contributes 0 — trends never rank as profit). LOOP-11 ADD 2026-09-25: research-feed PROVIDER BOUNDARY — `research/research-feed.provider.ts` `validateResearchSignal` (canonical factor/quality/value mapping, alias table, UNKNOWN→0 quality) + spec; establishes the external research-feed contract for later ingestion. NOT STARTED: trend_signals / opportunity_signals / opportunity_scores tables, opportunity normalization beyond evidence, Affiliate-04 profit check — Category B/later (needs external research feed + verified commission evidence for estimation).
- PHASE-03 Money/Affiliate Engine — SLICE 2026-09-25 (Owner MAXIMUM-AUTONOMY directive): Revenue Consolidation/Concentration KPI (Affiliate-07) IMPLEMENTED + TESTED (RevenueService.concentration(), GET /revenue-events/concentration — only reconciled/verified events count; riskAlert when one provider >90% of verified revenue; tests 124/124). REVENUE-CONCENTRATION DASHBOARD WIDGET DONE 2026-09-25 (final sweep) — DashboardService.overview() embeds concentration; web /dashboard renders provider share + risk alerts. LOOP-11 ADD 2026-09-25: PROVIDER-ADAPTER REGISTRY — `providers/provider-adapter.ts` (PROVIDER_ADAPTERS: product-data/manual, product-data/pa-api, affiliate-network/amazon-associates, affiliate-network/second-provider, automation/n8n, research-feed/trends), `provider-registry.service.ts` config validation (presence of documented env var NAMES only — never values), `system/system.controller.ts` GET /system/providers + /system/providers/:name (401-protected). NOT STARTED: second-provider adapter (EXPLICITLY gated behind Money-First MVP gate per Affiliate-07), offers/campaign/commission-event entities — Category B/later.
- PHASE-04 Content Factory — SLICE 2026-09-25 (Owner MAXIMUM-AUTONOMY directive): deterministic Content QA rules engine IMPLEMENTED + TESTED (content-qa.rules.ts — 8 checks across network-compliance + content-quality gates per 17_AMAZON_COMPLIANCE; GET /content-qa/assets/:id read-only). QA-01 APPROVAL-GATE ENFORCEMENT (safe) + full verification matrix 2026-09-25 — fresh unpublished→published transitions rejected (422 + verdict) unless BOTH QA gates pass; pre-approved whitelist protects live Campaign #3; prospective-state evaluation; already-published reaffirmation = no-op never gated. LOOP-11 ADD 2026-09-25: CONTENT FINGERPRINT FOUNDATION — `content-qa.fingerprint.ts` (SHA-256 32-hex digest over whitespace/case-normalized title+description+destination), `fingerprint` field on QaEvaluation, service integration + spec; provides the stored-fingerprint contract so once generated content exists, change-detection is ready. Still NOT STARTED: content generation, image workflows — Category B (external content/image APIs/pipeline).
- PHASE-05 through PHASE-12 — later phases (n8n automation, CEO memory, Owner Command Center, production hardening, controlled production, scale, continuous optimization). EXCEPTION 2026-09-25 (final sweep RESTART): PHASE-06 CAMPAIGN ANALYTICS SLICE IMPLEMENTED + TESTED — normalize affiliate outcomes into per-campaign metrics (links, clicks, conversions, reconciled revenue, net profit, provider count, channels) from RECORDED EVIDENCE only; GET /campaign-analytics/overview (read-only, deterministically grouped via link.campaign; untagged evidence → "(uncategorized)" bucket so totals always reconcile; pending/rejected events never count as conversions). Never infers reach/impressions (needs social API). tests 127/127. LOOP-11 ADD 2026-09-25: PHASE-08 COMMAND CENTER READ FOUNDATION — `boss.service.list()` now embeds the full plan tree (plan→tasks→actions) + audit trail; new web page `apps/web/app/command-center/page.tsx` renders commands→plans→actions + integration status (read-only, SSR, fetches /boss/commands + /system/providers under the API key). Remaining Phase-05 (n8n automation — external) + remaining Phase-06 (campaign entity normalisation — Category B/later).
- PHASE-09 SLICE 2026-09-25 (LOOP-11, Owner MAXIMUM-AUTONOMY directive): OBSERVABILITY + BACKUPS — `observability/http-exception.filter.ts` AllExceptionsFilter (consistent {statusCode,error,message,path,timestamp}; masks ALL ≥500 messages client-side, logs detail server-side) + `observability/request-logger.interceptor.ts` (formatLogLine + RequestLoggerInterceptor) wired via APP_FILTER/APP_INTERCEPTOR; `.env.example` now documents the provider credential NAMES contract (commented); `docs/09_BACKUP_RESTORE.md` runbook + `scripts/db-backup.mjs` + `scripts/db-restore-test.mjs` (npm run db:backup / db:restore-test) — LOCAL RESTORE DRILL PASSED 2026-09-25 (13/13 tables, every row count matches source; scratch DB dropped; source untouched).
...
### Moderate B — Phase-04 Content Factory remaining (content generation / image workflows)
TASK: Generate campaign content + images (+ fingerprint any generated content)
STATUS: BLOCKED (B — external API/pipeline)
WHY BLOCKED: generation needs an external content/image provider (text gen, image gen, or a manual authoring pipeline); fingerprints apply once generated content exists
...
LOCAL FOUNDATION ALREADY BUILT: YES — LOOP-11 added `content-qa.fingerprint.ts` (SHA-256 fingerprint over normalized title/description/destination) + `fingerprint` on every QaEvaluation; the QA rules engine + QA-01 gate already provide the acceptance contract. Change-detection (stored vs prospective fingerprint) is now testable end-to-end.
